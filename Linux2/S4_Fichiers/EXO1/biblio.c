...
bool ecrireFichier(Livre livre[], File* file){
   File* fileOpen = fopen(*file, "wb");
   if(fileOpen == NULL){
      perror("Problème fileOpen");
      return false;
      exit(1);
   }
   if(fwrite(&livre, sizeof(struct Livre), strlen(livre) ,fileOpen) != strlen(livre)){
